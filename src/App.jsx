...
export default function App() {
  const [menu, setMenu] = useState(false); const [piece, setPiece] = useState(null); const [scrolled, setScrolled] = useState(false); const heroVideoRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const syncToScroll = () => {
      ...
      const progress = Math.min(1, Math.max(0, -video.getBoundingClientRect().top / window.innerHeight));
      video.currentTime = video.duration * progress;
    };
    const onScroll = () => requestAnimationFrame(syncToScroll);
    video.addEventListener('loadedmetadata', syncToScroll);
    addEventListener('scroll', onScroll, { passive: true });
    syncToScroll();
    return () => { video.removeEventListener('loadedmetadata', syncToScroll); removeEventListener('scroll', onScroll); };
  }, []);
  ...
  return <>
    ...
    <main id="top"><section className="hero"><video ref={heroVideoRef} muted playsInline preload="auto" className="hero-video"><source src={heroVideo} type="video/mp4"/></video><div className="hero-shade"/><div className="hero-content"><p className="eyebrow light">Lagos · Nigeria</p><h1>Bespoke elegance,<br/><em>rooted in craft.</em></h1><p>Nigerian craftsmanship meets a global point of view.</p><div className="hero-actions"><a className="button light-button" href="#collection">Discover the collection</a><a className="text-link" href="#booking">Book a consultation →</a></div></div></section>
